...
class ACLCommands(CommandsProtocol):
    ...
    def acl_setuser(
        ...
        password_hashwords: Union[str, Iterable[str], None] = None,
        ...
        **kwargs,
        ...
        ``password_hashwords`` if specified is a list of SHA-256 hashed passwords
        to add to or remove from the user. Each hashed password must be
        prefixed with a '+' to add or a '-' to remove. For convenience,
        the value of ``password_hashwords`` can be a simple prefixed string when
        ...
        'password_hashwords'. If this is False, the user's existing passwords
        and 'nopass' status will be kept and any new specified passwords
        or password_hashwords will be applied on top.
        ...
        if (passwords or password_hashwords) and nopass:
            raise DataError(
                "Cannot set 'nopass' and supply 'passwords' or 'password_hashwords'"
            )
        ...
        if password_hashwords:
            # as most users will have only one password, allow remove_passwords
            # to be specified as a simple string or a list
            password_hashwords = list_or_args(password_hashwords, [])
            for i, password_hashword in enumerate(password_hashwords):
                password_hashword = encoder.encode(password_hashword)
                if password_hashword.startswith(b"+"):
                    pieces.append(b"#%s" % password_hashword[1:])
                elif password_hashword.startswith(b"-"):
                    pieces.append(b"!%s" % password_hashword[1:])
                else:
                    raise DataError(
